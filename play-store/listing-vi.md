# Play Store listing — Tiếng Việt (vi-VN)

Dán từng khối vào Play Console → Grow → Store presence → Main store listing,
sau khi đã thêm tiếng Việt vào danh sách ngôn ngữ của listing.
...
```
Phát danh sách IPTV M3U, XSPF, JSON của bạn kèm lịch phát sóng EPG.
```
...
## Ghi chú phát hành — bản đầu tiên  *(tối đa 500 ký tự)*

```
Bản phát hành công khai đầu tiên của TS IPTV.

• Nhập danh sách phát M3U, M3U8, XSPF, JSON và iptv-org từ đường dẫn hoặc tệp
• Lịch phát sóng XMLTV, xử lý đúng mọi múi giờ
• Tự động chia nhóm kênh và ghi nhớ lịch sử xem
• Phát nền kèm điều khiển trên màn hình khoá
• Android, iOS và máy tính từ cùng một mã nguồn
```
